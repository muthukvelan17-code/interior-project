    ...
    <div className={styles.authContainer}>
      ...
      >
        <div className={styles.authHeader}>
          ...
          <p className={styles.authSubtitle}>Sign in to InteriorConnect Tamil Nadu</p>
        </div>

        <div className={styles.roleToggle}>
          <button 
            className={`${styles.roleBtn} ${authMethod === 'email' ? styles.active : ''}`}
            onClick={() => setAuthMethod('email')}
          >
            Email Login
          </button>
          <button 
            className={`${styles.roleBtn} ${authMethod === 'phone' ? styles.active : ''}`}
            ...
            <>
              <div className={styles.inputGroup}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <span style={{ display: 'flex', alignItems: 'center', padding: '0 12px', border: '1px solid #ddd', borderRadius: '6px', backgroundColor: '#f9f9f9', color: 'var(--text-secondary)' }}>+91</span>
                  <input type="tel" placeholder="9876543210" className={styles.inputField} style={{ flex: 1 }} />
                </div>
              </div>
            </>
          )}
